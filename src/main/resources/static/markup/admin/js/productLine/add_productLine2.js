...
function previewImage1(){
	...
	reader.onload = function(r_event) {
	  document.getElementById('prev1').setAttribute('src', r_event.target.result);
	}
	document.getElementsByName('imgFile1')[0].addEventListener('change', function(event) {
...
function previewImage2(){
	...
	var reader = new FileReader();
	reader.onload = function(r_event) {
	  document.getElementById('prev2').setAttribute('src', r_event.target.result);
	}
	document.getElementsByName('imgFile2')[0].addEventListener('change', function(event) {
	    reader.readAsDataURL(this.files[0]);
	});
...
function previewImage3(){
	...
	var reader = new FileReader();
	reader.onload = function(r_event) {
	  document.getElementById('prev3').setAttribute('src', r_event.target.result);
	}
	document.getElementsByName('imgFile3')[0].addEventListener('change', function(event) {
	    reader.readAsDataURL(this.files[0]);
	});
